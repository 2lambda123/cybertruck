...
def showBoxes_1(filepath, boxes_list, color = (255, 0, 128), thickness = 2):
    ...
    cv2.imshow("Test Boxes" + filepath, img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

def showBoxes_2(filepath, boxes_list, centers, alt_center, color = (255, 0, 128), thickness = 2):
    img = cv2.imread(filepath)
    
    for box, center, alt_center in zip(boxes_list, centers, alt_center):
        cv2.rectangle(img, box[0], box[1], color, thickness)
        cv2.circle(img, center, 5, (0, 0, 255), -1)
        cv2.circle(img, alt_center, 5, (0, 220, 30), -1)
